...
type NoVisualOutputCache(visualOutputService: IVisualOutputService) =
    interface IVisualOutputCache with
        member _.Html(html) = visualOutputService.RenderHtml html
...
type FallbackVisualOutputService() =
    interface IVisualOutputService with
        member _.RenderHtml(html) =
            let isLikelyPlotly =
                html.Contains("Plotly.newPlot", StringComparison.OrdinalIgnoreCase)
                || html.Contains("plotly.js", StringComparison.OrdinalIgnoreCase)
                || html.Contains("plotly-", StringComparison.OrdinalIgnoreCase)

            if isLikelyPlotly then
                HtmlUnsupported "Plotly HTML output detected; Chrome/CDP visual rendering is not enabled yet."
            else
                HtmlUnsupported "HTML output detected; visual rendering is not enabled yet."
        ...
        task {
            let message =
                sprintf
                    ...
                    id
                    (JsonSerializer.Serialize(methodName))
                    paramsJson
    ...
    let contentClipParams (socket: ClientWebSocket) commandId (cancellationToken: CancellationToken) =
        task {
            let expression =
                """
(() => {
  const target = document.querySelector('.js-plotly-plot') || document.body.firstElementChild || document.body;
...
"""
            ...
            try
                ...
                let value = document.RootElement.GetProperty("result").GetProperty("result").GetProperty("value").GetString()
    ...
    let waitForVisualRender (socket: ClientWebSocket) commandId (cancellationToken: CancellationToken) =
        task {
            let expression =
                """
(() => !!document.querySelector('.js-plotly-plot .main-svg, .js-plotly-plot canvas, svg, canvas'))()
"""
            ...
            while not isReady && attempts > 0 do
                try
                    ...
                    use document = JsonDocument.Parse(response)
                    isReady <- document.RootElement.GetProperty("result").GetProperty("result").GetProperty("value").GetBoolean()
                with _ ->
                    isReady <- false
    ...
    let renderHtmlAsync html =
        task {
            if String.IsNullOrWhiteSpace chromePath || not (File.Exists chromePath || chromePath = "chrome.exe" || chromePath = "google-chrome") then
                return HtmlUnsupported(sprintf "Chrome executable was not found at '%s'." chromePath)
            else
                let port = availablePort ()
                let userDataDir = Path.Combine(Path.GetTempPath(), "lfsx-chrome-" + Guid.NewGuid().ToString("N"))
                let htmlPath = Path.Combine(Path.GetTempPath(), "lfsx-output-" + Guid.NewGuid().ToString("N") + ".html")
                ...
                let! renderResult =
                    task {
                        ...
                        try
                            ...
                            let! _ = sendCommand socket 3 "Emulation.setDeviceMetricsOverride" viewportParams cancellationToken
                            ...
                            let! screenshotResponse =
                                sendCommand socket (nextCommandId + 2) "Page.captureScreenshot" screenshotParams cancellationToken
                            ...
                            if root.TryGetProperty("error", &errorProperty) then
                                return HtmlUnsupported("Chrome/CDP rendering failed: " + errorProperty.GetProperty("message").GetString())
                            else
...
type KittyImageBackend(?maxChunkLength: int, ?reservedRows: int) =
    ...
    let reservedRowsFor (bytes: byte[]) =
        ...
        | Some(_, height) ->
            Math.Max(1, int (Math.Ceiling(float height / estimatedCellPixelHeight)))
        | None ->
            fallbackReservedRows
    ...
    let placementSequence imageId placementId sourceY sourceHeight =
        sprintf "%s_Ga=p,i=%d,p=%d,q=2,x=0,y=%d,h=%d,C=1;%s" escape imageId placementId sourceY sourceHeight stringTerminator

    let emitAt imageId placementId row column sourceY sourceHeight =
        match activePlacements.TryGetValue placementId with
        | true, (_, activeRow, activeColumn, activeSourceY, activeSourceHeight)
            when activeRow = row
                 && activeColumn = column
...
module OutputRendering =
    ...
    let renderOutput (theme: NotebookTheme) errorBrush (visualOutputCache: IVisualOutputCache) imageBackend output =
        ...
        | NotebookOutput.Display display ->
            ...
            | MimeTypes.Png, BinaryPayload bytes ->
                renderImage theme imageBackend { MimeType = MimeTypes.Png; Bytes = bytes }
            | MimeTypes.Svg, TextPayload svg ->
                textBlock theme.Muted ("[image/svg+xml]\n" + svg)
            | MimeTypes.PlotlyJson, TextPayload _ ->
                textBlock theme.Muted "[application/vnd.plotly.v1+json]\nPlotly JSON output detected; visual rendering is not enabled yet."
            | mimeType, TextPayload value ->
                textBlock theme.Muted (sprintf "[%s]\n%s" mimeType value)
            | mimeType, BinaryPayload bytes ->
                textBlock theme.Muted (binaryDescription mimeType bytes)
    ...
    let renderOutputs theme errorBrush visualOutputCache imageBackend outputs =
        ...
        |> List.iter (fun output ->
            panel.Children.Add(renderOutput theme errorBrush visualOutputCache imageBackend output) |> ignore)
