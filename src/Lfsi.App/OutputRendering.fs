...
(() => {
  const candidates = [
    ...document.querySelectorAll('.plotly-graph-div, .js-plotly-plot, svg, canvas, body > div, body')
  ];
    ...
    let waitForVisualRender (socket: ClientWebSocket) commandId (cancellationToken: CancellationToken) =
        task {
            let expression =
                """
(() => {
  const candidates = document.querySelectorAll('.plotly-graph-div, .js-plotly-plot, svg, canvas');
