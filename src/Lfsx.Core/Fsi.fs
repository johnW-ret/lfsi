...
type FsiSession(?workingDirectory: string) =
    ...
    let cleanOutput (marker: string) (text: string) =
        ...
        |> Array.map (fun line ->
            if line.StartsWith("> ", StringComparison.Ordinal) then line.Substring(2) else line)
        ...
        task {
            if proc.HasExited then
                ...
                if finished then
                    ...
                    let text =
                        if String.IsNullOrWhiteSpace errText then outText else errText

                    return { Input = code; Output = NotebookOutput.Error("Timed out waiting for fsi output.\n" + text.Trim()) }
        }
